...
export default function SettingsPage({
  ...
  const liveMetrics = [
    ...
    ['Đã cấu hình', moduleMetrics.configuredSettings ?? allItems.filter(item => item.status === 'active').length ?? 0]
  ]

  const settingsPanels = useMemo(() => [
    ['Platform Identity', 'school', [['Tên website', 'system'], ['Logo', 'website'], ['Favicon', 'website']]],
    ['Security & Access', 'admin_panel_settings', [['Admin auth', 'security'], ['Cookie session', 'security'], ['Role policy', 'role']]],
    ['Gamification', 'stars', [['XP', 'gamification'], ['Coin', 'gamification'], ['Level', 'gamification']]],
    ['Notifications', 'notifications', [['Email', 'notification'], ['Push', 'notification'], ['In-app', 'notification']]],
  ], [])
  const configuredTypes = useMemo(() => new Set(
    allItems.filter(item => item.status === 'active').map(item => item.type)
  ), [allItems])
  ...
  return (
    <section className={styles.section}>
      <div className={styles.sectionHeader}>
        ...
        <div className={styles.headerActions}>
          <button type="button" onClick={onOpenForm} className={styles.addBtn}>
            ...
            {moduleConfig.primaryAction || 'Thêm cấu hình'}
          </button>
      ...
      {/* Settings Grid */}
      <div className={styles.stitchSettingsGrid}>
        {settingsPanels.map(([title, icon, fields]) => (
          <article key={title} className={styles.settingsCard}>
            <h3>
              <MaterialIcon>{icon}</MaterialIcon> {title}
            </h3>
            {fields.map(([field, type]) => (
              <div key={field} className={styles.settingsRow}>
                <span>{field}</span>
                <strong>{configuredTypes.has(type) ? 'Configured' : 'Not set'}</strong>
              </div>
            ))}
          </article>
        ))}
      ...
      <div className={styles.modulePanel}>
        ...
        <div className={styles.moduleTable}>
          {loading ? (
            <div className={styles.loader}><div className={styles.spinner} /> Đang tải Cài đặt...</div>
            ...
            <div className={styles.cmsEmpty}>
              <span><MaterialIcon>settings</MaterialIcon></span>
              <strong>Chưa có cấu hình cài đặt nào</strong>
              <p>Tạo record đầu tiên cho module này để cấu hình.</p>
              <button type="button" onClick={onOpenForm} className={styles.addBtn}>
                <MaterialIcon>add</MaterialIcon>
                {moduleConfig.primaryAction || 'Thêm cấu hình'}
              </button>
