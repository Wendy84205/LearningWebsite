...
export default function AchievementsPage({
  ...
  const rows = useMemo(() => {
    if (allItems.length > 0) {
      return allItems.slice(0, 6).map(item => ({
        ...
        detail: [item.grade, item.subject, item.topic, item.skill].filter(Boolean).join(' / ') || 'achievements',
  ...
  return (
    <section className={styles.section}>
      <div className={styles.sectionHeader}>
        ...
        <div className={styles.headerActions}>
          <button type="button" onClick={onOpenForm} className={styles.addBtn}>
            ...
            {moduleConfig.primaryAction || 'Thêm huy hiệu'}
          </button>
      ...
      {/* Achievements Stitch Grid Layout */}
      <div className={styles.stitchAchievementGrid}>
        {rows.slice(0, 6).map((row, index) => (
          <article key={row.id} className={styles.achievementCard}>
            <span className={styles.achievementIcon}>
              <MaterialIcon filled>{['workspace_premium', 'military_tech', 'emoji_events', 'stars'][index % 4]}</MaterialIcon>
            </span>
            <h3>{row.title}</h3>
            <p>{row.desc}</p>
      ...
      <div className={styles.modulePanel}>
        <div className={styles.modulePanelHeader}>
          <div>
            <h3>Platform Badges & Achievements</h3>
            <p>CRUD dữ liệu huy hiệu, huy chương Boss và các điều kiện mở khóa nhận thưởng</p>
          </div>
        ...
        <div className={styles.cmsToolbar}>
          <div className={styles.searchBox}>
            ...
            <input
              type="text"
              placeholder="Tìm trong Achievements..."
              value={search}
        ...
        <div className={styles.moduleTable}>
          {loading ? (
            <div className={styles.loader}><div className={styles.spinner} /> Đang tải Achievements...</div>
            ...
            <div className={styles.cmsEmpty}>
              <span>
                <MaterialIcon filled>emoji_events</MaterialIcon>
              </span>
              <strong>Chưa có huy hiệu nào</strong>
              <p>Thêm huy hiệu đầu tiên để thiết lập kinh tế thưởng cho bé.</p>
              <button type="button" onClick={onOpenForm} className={styles.addBtn}>
                <MaterialIcon>add</MaterialIcon>
                {moduleConfig.primaryAction || 'Thêm huy hiệu'}
              </button>
